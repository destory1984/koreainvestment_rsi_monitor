...
RSI를 대신 보고 있다가 알려주는 예쁜 목소리의 비서가 필요했다.
...
알림 목소리는 있는 것 가운데 가장 좋은 것을 저절로 고른다. **어느 것도 키나 가입이 필요 없다.**
...
미리 만들어 `tts_cache/` 에 쌓아 두고, 그 뒤로는 파일만 튼다.

Edge 음성은 Microsoft Edge 브라우저의 「소리 내어 읽기」가 쓰는 음성을 빌려 쓰는 것이라(`edge-tts` 꾸러미),
정식 서비스가 아니어서 언젠가 막힐 수 있다. 막히면 윈도우에 들어 있는 음성으로 읽는다.
...
- **지수 띠** — 제목 아래 한 줄에 S&P500 · 나스닥 종합 · 코스피 · 코스닥 · 니케이 · 원/달러 · 비트코인의
...
- **소리 알림** — RSI 가 35/65 를 넘으면 말머리 소리(땡) 뒤에 「테슬라 65 초과」, 30/70 을 넘으면
  ...
  위의 🔊 로 끄고 켜고, 「TTS 시험」으로 한 번 들어 본다. 울린 것과 억제된 것은 표 아래 「알림」에 쌓인다.
  소리는 Windows 에서만 난다.
...
- **매수·매도 시그널** — 봉이 닫힐 때마다 [webull_rsi_monitor](https://github.com/destory1984/webull_rsi_monitor) 의
  ...
  「테슬라 매수 시그널」처럼 말로도 알린다 (🔊 시그널 로 끄고 켠다).
