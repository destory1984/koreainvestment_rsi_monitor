...
- **지수 띠** — 제목 아래 한 줄에 S&P500 · 나스닥 종합 · 코스피 · 니케이 · 원/달러 · 비트코인의
...
- **소리 알림** — RSI 가 35/65 를 넘으면 말머리 소리(땡) 뒤에 「테슬라 65 초과」, 30/70 을 넘으면
  ...
  위의 「⚙ 설정」에서 끄고 켜고, 「TTS 테스트」로 한 번 들어 본다. 소리가 꺼져 있으면 설정 버튼에 🔇 가 붙는다. 울린 것과 억제된 것은 표 아래 「알림」에 쌓인다.
  소리는 Windows 에서만 난다.
...
- **매수·매도 시그널** — 봉이 닫힐 때마다 [webull_rsi_monitor](https://github.com/destory1984/webull_rsi_monitor) 의
  ...
  「테슬라 매수 시그널」처럼 말로도 알린다 (「⚙ 설정」의 시그널 소리로 끄고 켠다).
...
주간거래 체결도 분봉·RSI·알림에 넣는다 (Webull 5분봉도 주간거래 봉을 넣는다). 주간거래 체결가일 때는 가격 앞에 「주간」이 붙는다.
켤 때 주간거래 분봉(`BAQ` 등)도 받아 끼워 넣는데, 못 받으면 켠 뒤의 체결만 쌓이니 한동안 Webull 과 조금 다를 수 있다.
