...
(예전 JSON 캐시는 처음 돌릴 때 옮겨 담고 원본은 `replay_cache/json_backup/` 에 둔다.) 국내 종목은 1분봉을 묶어 한 달 넘게 거슬러 받는다 (3주에 25초쯤).
...
국내 분봉(KRX 정규장만), 텔레그램, 분봉 DB, 채점을 가짜 데이터로 시험한다 (37개, 1초 안). 한국투자증권에 접속하지 않고
