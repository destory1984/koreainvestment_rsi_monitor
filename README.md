...
python kis_replay.py --by 세션         # 프리·정규·애프터·주간(국내는 정규·NXT) 따로 (--by 종목 도 된다)
...
국내 분봉에는 넥스트레이드 애프터(15:30~20:00)도 들어 있어 「NXT」 세션으로 따로 센다.
웹 화면의 국내 실시간은 KRX 만이라 지금은 NXT 시간에 알림이 나지 않는다.
