...
미리 만들어 `tts_cache/` 에 쌓아 두므로, 그 뒤로는 인터넷이 끊겨도 파일로 울린다.

Edge 음성은 Microsoft Edge 브라우저의 「소리 내어 읽기」가 쓰는 음성을 빌려 쓰는 것이라(`edge-tts` 꾸러미),
정식 서비스가 아니어서 언젠가 막힐 수 있다. 막히면 윈도우 음성으로 넘어간다.
윈도우 음성에 한국어가 없으면 윈도우 설정 → 시간 및 언어 → 음성 → 음성 추가에서 한국어를 더한다.
...
- **소리 알림** — RSI 가 35/65 를 넘으면 말머리 소리(땡) 뒤에 「테슬라 65 초과」, 30/70 을 넘으면
  ...
  소리는 Windows 에서만 난다.
- **목소리** — 로컬 TTS → Edge 음성 → 윈도우 음성 차례로 고른다 ([5. 목소리](#5-목소리-따로-할-것-없음)).
