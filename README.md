...
- **지수 띠** — 제목 아래 한 줄에 S&P500 · 나스닥 종합 · 코스피 · 원/달러 · 비트코인 · 니케이의
  ...
  공휴일에는 「휴장 (휴일)」. 장이 없는 날(주말·휴일)에는 그 시장 종목 줄에 이름·가격·등락만 두고 「휴장」이라 쓰고, 알림·시그널도 보지 않는다. 국내 휴장일은 한국투자증권에 하루 한 번 묻고(`kis_holidays.json`),
  미국 휴장일은 `kis_rsi.py` 의 `US_HOLIDAYS` 에 2027년까지 적어 두었다 (일찍 닫는 날은 모른다).
...
- **소리 알림** — RSI 가 35/65 를 넘으면 말머리 소리(땡) 뒤에 「테슬라 65 초과」, 30/70 을 넘으면
  ...
  「⚙ 설정」의 「소리 나는 때」에서 미국 주간거래·프리장·정규장·애프터·국내 종목마다 소리를 끄고 켜고,
  「조용한 시각」(이 PC 시각, 23:00~07:00 처럼 자정을 넘어도 된다)을 정한다. 가린 알림도 기록에는 🔇 와 함께 쌓인다.
...
- **매수·매도 시그널** — 봉이 닫힐 때마다 본다 (`kis_signal.py`). RSI 가 35 이하(65 이상)로 가면 12봉 동안 매수(매도)
  ...
  처음에는 [webull_rsi_monitor](https://github.com/destory1984/webull_rsi_monitor) 의 `rsi_signal.py` 처럼
  「MACD 히스토그램 부호가 바뀔 때」 냈는데, 꼭대기·바닥에서 30분(중앙값)쯤 늦어 09-25 에 바꿨다.
  쌓인 분봉으로 견주면 5분쯤으로 빨라지고 60분 뒤 맞음은 비슷하다 (둘 다 50% 언저리).
  표의 「매수/매도」 칸에 마지막 시그널과 그 봉 시각이, 차트에는 ▲매수 ▼매도 가 붙는다.
  「테슬라 매수 시그널」처럼 말로도 알린다 (「⚙ 설정」의 「매수·매도 시그널」로 끄고 켠다).
  (옛 규칙일 때 2026-09-24 장중 11종목에서 Webull 버전이 낸 시그널 15개 가운데 13개가 같은 봉·같은 방향으로 나왔다.)
...
켤 때는 DB 에 쌓인 지난 5일 치를 받은 봉 앞에 이어 붙여 차트를 며칠 뒤까지 볼 수 있다 (`kis_web.py` 의 `HISTORY_DAYS`). 국내 종목은 1분봉을 묶어 한 달 넘게 거슬러 받는다 (3주에 25초쯤).
국내는 웹 화면과 같이 KRX 정규장(09:00~15:30) 봉만 쓴다.
...
```bash
python kis_replay.py --collect        # 주간거래 분봉만 받아 replay_cache/bars.db 에 쌓고 끝난다 (몇 초)
```
...
국내 분봉(KRX 정규장만), 텔레그램, 분봉 DB, 채점을 가짜 데이터로 시험한다 (48개, 1초 안). 한국투자증권에 접속하지 않고
