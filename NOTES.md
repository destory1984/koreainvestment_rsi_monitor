...
20. **국내 종목 되감기** (09-25) — `kis_replay.py` 가 국내도 한다. 1분봉을 `fetch_kr_bars(need=days*133)` 로 받아 캐시에 합친다.
    세션은 「정규」(~15:30 종가 단일가 봉까지)·「NXT」(넥스트레이드 애프터, SK하이닉스는 09-14 부터 보인다).
...
국내 (09-25, SK하이닉스·삼성전자, 08-26~09-23, 60분 뒤, 보합 뺀 맞음): 30 미만 54%·−0.00, 35 미만 37%·−0.16,
65 초과 51%·−0.03, 70 초과 54%·+0.10, 매수 강 50%·−0.01, 매수 약 31%·−0.26, 매도 강 33%·−0.12.
국내는 알림·시그널 모두 값어치가 거의 없고 매수 약·매도 강은 거꾸로다. 두 종목뿐이라 믿을 만하지 않다.
...
- **국내 실시간 체결** — 연휴라 못 봤다. **2026-09-28(월) 장중**에 삼성전자·SK하이닉스 체결, H0STCNT0 필드가 맞는지.
  국내 분봉에는 넥스트레이드(~20:00)가 들어오는데 실시간은 KRX 만이라 15:30 뒤 값이 멈춘다.
